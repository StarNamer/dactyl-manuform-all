...
(def β (/ π 34))                        ; curvature of the rows
(def centerrow (- nrows 4.25))             ; controls front-back tilt
...
(defn column-offset [column] (cond
  ...
  (= column 4) [50 0 -7]
  (= column 5) [50 0 -7]
  (>= column 6) [50 0 1]             ; original [0 -5.8 5.64]
  :else [0 0 0]))

(def thumb-offsets [-2 -2 2])

(def keyboard-z-offset 27)               ; controls overall height; original=9 with centercol=3; use 16 for centercol=2
...
+(def fixed-x [-42 -22 0 22 44 66 107 129 151])  ; relative to the middle finger
