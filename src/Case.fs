...
let topWall =
    ...
    let tempBlock = 
        jointBlock
        |> bla (float(lastcol)) -1.0 0.0 0.0


    [ [for x in 0.0 .. ncols - 1.0 do key_wall_brace (float(x)) -1.0 0.0 1.0 web_post_bl (float(x)) -1.0 0.0 1.0 web_post_br] |> List.collect id
    ; [for x in 1.0 .. ncols - 1.0 do key_wall_brace (float(x)) -1.0 0.0 1.0 web_post_bl (float(x - 1.0)) -1.0 0.0 1.0 web_post_br] |> List.collect id
    ; jointBolts
    ] |> List.collect id
...
let frontWall =
    ...
    let jointBolts =
        ...
        [ key_place 4.0 4.0 (jbl jointBlock -1.2)
        ; key_place 5.0 4.0 (jbl jointBlock -1.2)
        ; key_place 3.0 5.0 (jbl jointBlock -1.2)
        ; key_place 2.0 5.0 (jbl jointBlock -1.3)
        ] |> List.collect id

    [ [for x in 4.0 .. ncols - 1.0 do key_wall_brace (float(x)) lastrow 0.0 -1.0 web_post_tl (float(x)) lastrow 0.0 -1.0 web_post_tr] |> List.collect id
