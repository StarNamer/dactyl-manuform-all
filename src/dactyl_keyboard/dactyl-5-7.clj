...
(def case-walls
  (union
   ...
   (for [x (range (dec ncols) ncols)] (key-wall-brace x 0 0 1 web-post-tl x       0 0 1 wide-post-tr))
                       ...
                       (if shift-down  (key-position column row (map - (wall-locate2  0 -2.5) [0 (/ mount-height 2) 0]))
                        (if shift-thumb  (map + (left-key-position row 0) (wall-locate3 0.9 0.3))
                         (if shift-left (map + (left-key-position row 0) (wall-locate3 -1 0))
                                        (key-position column row (map + (wall-locate2  3.6  0) [(/ mount-width 2) 0 0]))))))
        ]
