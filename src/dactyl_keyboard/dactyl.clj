...
(def centercol 6)                       ; controls left-right tilt / tenting (higher number is more tenting)
...
(defn column-offset [column] (cond
  (= column 2) [0 2.82 -4.5]
  (>= column 4) [0 -3.2 5.64]            ; original [0 -5.8 5.64]
  :else [0 0 0]))
...
(def case-walls
  (union
   ...
   ; left wall
   (for [y (range 0 (inc lastrow))] (union (wall-brace (partial left-key-place y 1)       -1 0 web-post (partial left-key-place y -1) -1 0 web-post)
                                     (hull (key-place 0 y web-post-tl)
                                           (key-place 0 y web-post-bl)
                                           (left-key-place y  1 web-post)
                                           (left-key-place y -1 web-post))))
   (for [y (range 1 (inc lastrow))] (union (wall-brace (partial left-key-place (dec y) -1) -1 0 web-post (partial left-key-place y  1) -1 0 web-post)
                                     (hull (key-place 0 y       web-post-tl)
...
(spit "things/print-right-walls.scad"
      (write-scad
       (difference case-walls usb-holder-hole)))

(spit "things/print-right-plate.scad"
      (write-scad
       (union key-holes connectors)))

(spit "things/print-right-thumb-plate.scad"
      (write-scad
       (union thumb thumb-connectors thumbcaps)))
