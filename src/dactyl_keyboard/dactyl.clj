...
(def bottom-plate-infill-thickness 3)
...
;;;;;;;;;;;;;;;;;
;;
;; plate -> part around switch hole
;; retention-tab -> holds the key (the real plate)
;;
;;

(def keyswitch-height 14.2) ;; Was 14.1, then 14.25, then 14.4
...
(def left-wall-x-offset 8) ; original 10
...
(def front-wall
  (let [tr (if (true? pinky-15u) wide-post-tr web-post-tr)
        br (if (true? pinky-15u) wide-post-br web-post-br)]
    (union (key-wall-brace 3 lastrow   0 -1 web-post-bl 3 lastrow 0.5 -1 web-post-br)
           (key-wall-brace 3 lastrow 0.5 -1 web-post-br 4 cornerrow 0.5 -1 web-post-bl)
           (for [x (range 4 ncols)] (key-wall-brace x cornerrow 0 -1 web-post-bl x      cornerrow 0 -1 br)) ; TODO fix extra wall
...
(def back-wall
  (let [tr (if (true? pinky-15u) wide-post-tr web-post-tr)
        br (if (true? pinky-15u) wide-post-br web-post-br)]
    (union (key-wall-brace (dec ncols) 0 0 1 web-post-tl (dec ncols) 0 0 1 tr)
...
(def usb-holder-cube   (cube 23 12 2)) ; 23x8.5x2 size of usb jack holder
(def usb-holder-space  (translate (map + usb-holder-position [0 (* -1 wall-thickness) 1]) usb-holder-cube))
(def usb-holder-holder (translate (map + usb-holder-position [0 -2 0])  (cube 28 16 4)))
(def usb-jack (translate (map + usb-holder-position [0 10 2]) (cube 8.1 20 3.1))) ; outer size of usb micro jack

(defn usb-screw-insert [position shape height] 
  (translate (map + usb-holder-position [(first position) (second position) (- (/ height 2) 2)]) shape))


(defn usb-screw-insert-all [shape height]
  (union (usb-screw-insert [7.75 -2.5] shape height)
  (usb-screw-insert [-7.75 -2.5] shape height)
  )
)

(def usb-screw-holes  (usb-screw-insert-all (with-fn 12 (cylinder 1.7 10)) 10))
(def usb-screw-head-holes (usb-screw-insert-all (cylinder [3.2 1.7] 1.7) 1.7))


(def oled-width 12.5)
(def oled-length 38.4)
(def oled-height 7)
(def oled-cube (cube oled-length oled-width oled-height))

(def oled-holder-width  (+ 3 oled-width))
(def oled-holder-length (+ 3 oled-length))
(def oled-holder-height (+ 1 oled-height))
(def oled-holder-cube (translate [0 0 1] (cube oled-holder-length oled-holder-width oled-holder-height)))

(def oled-holder-cube-2 (cube oled-holder-length oled-holder-width (+ 5 oled-holder-height)))
(def oled-bottom (translate [0 (- (/ oled-holder-width 2) 1.5) (* -1 oled-height)] (cube oled-holder-length 3 1)))
(def oled-cube-2 (translate [0 0 -1] (cube oled-length oled-width (+ 6 oled-height))))

(def oled-va-width 7.58)
(def oled-va-length 24.38)
(def oled-va-right 7.51)
(def oled-va-left (- (- oled-length oled-va-right) oled-va-length)) ; 6.6
(def oled-va-x-offset (- oled-va-left oled-va-right))
(def oled-view-height (+ 3 oled-height))
(def oled-view-cube (translate [oled-va-x-offset 0 2] (cube oled-va-length oled-va-width oled-view-height)))

(def oled-holder-ref (key-position 5 -1 (map - (wall-locate2  0  0) [(/ mount-width 2) mount-height 0])))
(def oled-holder-position (map + [-1 2 29] [(first oled-holder-ref) (second oled-holder-ref) 2]))

(def oled-rotate-x-deg 30)
(def oled-rotate-y-deg 6)

(def oled-view (translate (map + oled-holder-position [0 0 0]) 
    (->> oled-view-cube
    (rotate (deg2rad oled-rotate-x-deg) [1 0 0]) (rotate (deg2rad oled-rotate-y-deg) [0 1 0])
    )))
    
(def oled-holder-hole (translate (map + oled-holder-position [0 0 0]) 
    (->> oled-cube 
    (rotate (deg2rad oled-rotate-x-deg) [1 0 0]) (rotate (deg2rad oled-rotate-y-deg) [0 1 0])
    )))
        
(def oled-holder (translate oled-holder-position 
    (->> (difference oled-holder-cube (union oled-cube oled-view-cube))
    (rotate (deg2rad oled-rotate-x-deg) [1 0 0]) (rotate (deg2rad oled-rotate-y-deg) [0 1 0])
    )))
    
(def oled-holder-1 (translate oled-holder-position 
    (->> (translate [0 0.45 -5.2] (difference oled-holder-cube-2 oled-cube-2))
    (rotate (deg2rad oled-rotate-y-deg) [0 1 0]))))

(def oled-holder-hole-1 (translate oled-holder-position 
    (->> (translate [0 0.45 -5.2] oled-cube-2)     
    (rotate (deg2rad oled-rotate-y-deg) [0 1 0]))
    ))

(def oled-bottom-bar (translate oled-holder-position 
    (->> (translate [0 0.45 -5.1] oled-bottom)
    (rotate (deg2rad oled-rotate-y-deg) [0 1 0]))))

(def teensy-width 28)  
(def teensy-height 12)
(def teensy-length 84)
(def teensy-pcb-thickness 1.6) 
(def teensy-holder-width  (+ 2 teensy-pcb-thickness))
...
(defn screw-insert [column row shape height offset] 
  (let [shift-right   (= column lastcol)
    ...
    (translate (map + offset [(first position) (second position) (/ height 2)]) shape)
  ))

(defn screw-insert-all [shape height]
  (union (screw-insert 0 0                shape height [5 0 0]) ;; left wall
         (screw-insert 0 (- lastrow 1)    shape height [5 0 0]) ;; left wall   
         (screw-insert 3 lastrow          shape height [0 1 0]) ;; column 3
         (screw-insert 2 0                shape height [0 -1 0]) ;; column 2
         ;(screw-insert (+ lastcol 0.5) 1.5        shape height) ;; center of right wall
         (screw-insert lastcol -0.4          shape height [0 0 0]) ;; right wall
         (screw-insert lastcol (- lastrow 0.6)    shape height [0 0 0]) ;; right wall
         (screw-insert 0 lastrow         shape height [-4 -5 0]) ;; thumb, lower
         (screw-insert 1 lastrow         shape height [-1 -15 0]) ;; thumb, upper
  ))

(defn screw-insert-all-shapes [bottom-radius top-radius height]
  (screw-insert-all (screw-insert-shape bottom-radius top-radius height) height))
(def screw-insert-height 9) ; was 3.8
(def screw-insert-bottom-radius (/ 4.8 2)) ; was 5.31 2
(def screw-insert-top-radius (/ 4.7 2)) ; was 5.1 2
...
(def pcb-holder-angle 87)        
(def pcb-holder-position [-40 40 0])  
(def pcb-holder-cube (cube 88 30 2))
(def pcb-holder (->> (translate (map + pcb-holder-position [40.5 12 bottom-plate-thickness]) pcb-holder-cube) (rotate (deg2rad pcb-holder-angle) [0 0 1])))
        
(defn pcb-screw-insert [position shape height] 
  (translate (map + pcb-holder-position [(first position) (second position) (/ height 2)]) shape))

(defn pcb-screw-insert-all [shape height]
  (union (pcb-screw-insert [0 0] shape height)
  (pcb-screw-insert [45 0] shape height)
  (pcb-screw-insert [81 0] shape height)
  (pcb-screw-insert [0 24] shape height)
  (pcb-screw-insert [45 24] shape height)
  (pcb-screw-insert [81 24] shape height)
  )
)

(def pcb-screw-holes  (->> (pcb-screw-insert-all (with-fn 12 (cylinder 1.7 100)) 100) 
                    (rotate (deg2rad pcb-holder-angle) [0 0 1])))
(def pcb-screw-head-holes (->> (pcb-screw-insert-all (cylinder [2.6 1.2] 1.2) 1.2)
                    (rotate (deg2rad pcb-holder-angle) [0 0 1])))
        
(def weightplate-thickness 2)
(def weightplate-position [15 0 0])  
(def weightplate-cube (cube 80 100 2))
(def weightplate-holder (translate (map + weightplate-position [0 0 (* 1 (- bottom-plate-thickness weightplate-thickness ))]) weightplate-cube))
...
(def plate-infill
  (union
    ...
    (triangle-hulls
      (translate [0 -23 0] (thumb-ml-place thumb-post-tl))
      (translate [15 -22 0] (thumb-ml-place thumb-post-bl))
      (translate [-8 0 0] (thumb-tl-place thumb-post-tl))
      (translate [30 0 0] (thumb-tl-place thumb-post-bl))
      (translate [30 10 0] (thumb-tr-place thumb-post-tr))
      (translate [30 10 0] (thumb-tr-place thumb-post-br))
    )
  ))   
...
(def plate-right
  (difference
    ...
    (union
      ;(color [10/255 230/255 80/255 1] (translate [0 0 -10] pcb-holder))
      ;(color [10/255 230/255 230/255 1] (translate [0 0 -10] weightplate-holder))
      (translate [0 0 -10] screw-holes)
      (translate [0 0 -10] screw-head-holes)
      (translate [0 0 -10] pcb-screw-holes)
      (translate [0 0 -10] pcb-screw-head-holes)
    )
  ))
  
(def plate-outline  
    (bottom bottom-plate-thickness (cut
        (translate [0 0 -0.1]
...
(def plate-test   
  (union
     ...
     (translate [0 0 25] (difference    
        plate-right plate-outline
       ))
     (translate [0 0 50]  plate-outline)
))
...
(spit "things/usb-holder.scad"
      (write-scad
         (difference usb-holder-holder usb-holder-space usb-jack (union usb-screw-holes usb-screw-head-holes))
))

(spit "things/oled-holder.scad"
      (write-scad (union oled-bottom-bar 
                       (difference 
                               (union 
                                oled-holder    
                                oled-holder-1
