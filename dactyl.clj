...
(def α (/ π 28))                        ; curvature of the columns
(def β (/ π 25))                        ; curvature of the rows
(def centerrow (- nrows 5.3))             ; controls front-back tilt
(def centercol 5.5)                       ; controls left-right tilt / tenting (higher number is more tenting)
(def tenting-angle (/ π 14))            ; or, change this for more precise tenting control
(def column-style :orthographic)

(def thumb-offsets [-3 -5 3])
...
(def wall-thickness 6)                  ; wall thickness parameter; originally 5
...
(def numpad-key-dist 19.5)

(def numpad-height -2.5)
...
(defn number-column [column] (>= column (- ncols 3)))

(defn column-angle-f [column] (if (number-column column)
                                (- tenting-angle)
                                (* β (- centercol column))))

(defn column-z-f [column] (if (number-column column)
                            0
                            (* column-radius (- 1 (Math/cos (column-angle-f column))))))
...
(defn column-offset [column] (cond
                               ...
                               (= column 4) [41 -18 -7]
                               (= column 5) [39.9 -18 -7]
                               (= column 6) [45 -18 numpad-height]
                               (= column 7) [42 -18 numpad-height]
                               (= column 8) [39 -18 numpad-height]
                               :else [0 0 0]))

(defn apply-key-geometry [translate-fn rotate-x-fn rotate-y-fn column row shape]
  (let [column-angle (column-angle-f column)
        rotate-y-if-needed (fn [angle obj] (if (number-column column)
                                             obj
                                             (rotate-y-fn column-angle obj)))
        transform-row (fn [obj] (if (number-column column)
                                  (translate-fn [0 (* numpad-key-dist (- centerrow row)) 0] obj)
                                  (rotate-x-fn (* α (- centerrow row)) obj)))
        placed-shape (->> shape
                          (translate-fn [0 0 (- row-radius)])
                          (transform-row)
                          (translate-fn [0 0 row-radius])
                          (translate-fn [0 0 (- column-radius)])
                          (rotate-y-if-needed column-angle)
                          (translate-fn [0 0 column-radius])
                          (translate-fn (column-offset column)))
        column-z-delta (column-z-f column)
...
(def usb-holder-position (key-position 7 0 (map + (wall-locate2 0 1) [0 (/ mount-height 2) 0])))
(def usb-holder-size [13 10.0 7])
